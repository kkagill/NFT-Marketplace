...
const App = {
  ...
  createToken: async function () {   
    ...
    if (confirm("토큰화시키는데 0.2 KLAY를 지불해야 합니다. 진행하시겠습니까?")) {          
      try {
        ...
        this.mintYTT(videoId, author, dateCreated, res[0].hash);     
      } catch (err) {
        console.error(err);
        spinner.stop();
      } 
  ...
  mintYTT: async function (videoId, author, dateCreated, hash) {    
    ...
    const feePayer = caver.klay.accounts.wallet.add('0x637b99fed337950a46dd4b147382fac294a86bcae9cb8e0cb88e2a7814333cb3')
    ...
    const { rawTransaction: senderRawTransaction } = await caver.klay.accounts.signTransaction({
      ...
      gas:  '500000',
      value: caver.utils.toPeb('0.2', 'KLAY'),
    }, sender.privateKey)
  ...
  renderMyTokensSale: function (tokenId, ytt, metadata, price) { 
    ...
    template.find('.on-sale').text(caver.utils.fromPeb(price, 'KLAY') + " KLAY에 판매 중");              
